...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getDietTemplateData({ calorie, goal: "mass", mealCount, dietType });
  const path = getDietPagePath({ calorie, goal: "mass", mealCount, dietType });
...
export default async function DietMassMealsTypePage({ params }: Props) {
  const { calorie: calorieSegment, meals: mealsSegment, type } = await params;
  const calorie = parseCalorieSegment(calorieSegment);
  const mealCount = parseMealsSegment(mealsSegment);
  const dietType = parseDietTypeSegment(type);
  ...
  const templateData = getDietTemplateData({ calorie, goal: "mass", mealCount, dietType });
  const oldSlug = dietParamsToSlug({ calorie, goal: "mass", mealCount });
  ...
  const entry: PublicBlogEntry = {
    id: `programmatic-diet-mass-${calorie}-${mealCount}-${dietType}`,
    slug: path,
    href: `/dieta/${path}`,
    title: templateData.title,
    h1: templateData.h1,
    description: templateData.description,
    category: "Dieta na masę",
