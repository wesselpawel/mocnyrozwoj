...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getDietTemplateData({ calorie, goal: "mass", mealCount });
  const path = getDietPagePath({ calorie, goal: "mass", mealCount });
...
export default async function DietMassMealsPage({ params }: Props) {
  const { calorie: calorieSegment, meals: mealsSegment } = await params;
  const calorie = parseCalorieSegment(calorieSegment);
  const mealCount = parseMealsSegment(mealsSegment);
  if (calorie === null || mealCount === null) notFound();

  const templateData = getDietTemplateData({ calorie, goal: "mass", mealCount });
  ...
  const entry: PublicBlogEntry = {
    id: `programmatic-diet-mass-${calorie}-${mealCount}`,
    slug: path,
    href: `/dieta/${path}`,
