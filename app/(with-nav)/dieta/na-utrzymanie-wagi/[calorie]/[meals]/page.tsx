...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = getDietPagePath({ calorie, goal: "maintenance", mealCount });
...
export default async function DietMaintenanceMealsPage({ params }: Props) {
  const { calorie: calorieSegment, meals: mealsSegment } = await params;
  const calorie = parseCalorieSegment(calorieSegment);
  const mealCount = parseMealsSegment(mealsSegment);
  ...
  const entry: PublicBlogEntry = {
    id: `programmatic-diet-maintenance-${calorie}-${mealCount}`,
    slug: path,
    href: `/dieta/${path}`,
