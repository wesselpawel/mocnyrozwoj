...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getDietTemplateData({ calorie, goal: "reduction", mealCount, dietType });
  const path = getDietPagePath({ calorie, goal: "reduction", mealCount, dietType });
...
export default async function DietReductionMealsTypePage({ params }: Props) {
  const { calorie: calorieSegment, meals: mealsSegment, type } = await params;
  const calorie = parseCalorieSegment(calorieSegment);
  const mealCount = parseMealsSegment(mealsSegment);
  const dietType = parseDietTypeSegment(type);
  ...
  const templateData = getDietTemplateData({ calorie, goal: "reduction", mealCount, dietType });
  const oldSlug = dietParamsToSlug({ calorie, goal: "reduction", mealCount });
  ...
  const entry: PublicBlogEntry = {
    id: `programmatic-diet-reduction-${calorie}-${mealCount}-${dietType}`,
    slug: path,
    href: `/dieta/${path}`,
