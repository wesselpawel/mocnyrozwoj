...
export default function AddDietPage() {
  ...
  const handleGenerateDetailed = async (section: string) => {
    if (!topic.trim()) {
      setErrorMessage("Proszę wprowadzić temat diety");
      return;
    }
    ...
    try {
      setErrorMessage("");
      const detailedData = await dietService.generateDetailedDietSection(
        topic,
        section,
        dietData
      );

      // Update diet data with detailed content
      setDietData((prev) => ({
        ...prev,
        ...detailedData,
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dietDataForDb = {
        ...
        shortDescription: dietData.shortDescription || "",
        ...
        nutritionistBio: dietData.nutritionistBio || "",

        // Detailed diet information
        dietOverview: dietData.dietOverview || "",
        benefits: dietData.benefits || [],
        contraindications: dietData.contraindications || [],
      ...
      // Save to database
      await dietService.addDiet(dietDataForDb);
    ...
    <div className="p-6 lg:p-16">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Utwórz nowy plan dietetyczny z wszystkimi niezbędnymi informacjami
        </p>
      </div>
